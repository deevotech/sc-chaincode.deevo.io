...
func TestFood_UpdateTraceable(t *testing.T) {
	scc := new(FoodChaincode)
	stub := shim.NewMockStub("food", scc)
	...
	updatedOrg := Traceable{ObjectType: "org", ID: "org_1", Name: "org 1", Content: "address 2"}
	updatedOrgAsBytes, err := json.Marshal(updatedOrg)
	if err != nil {
		fmt.Println("Failed to encode json")
		t.FailNow()
	}
	checkUpdateTraceable(t, stub, updatedOrgAsBytes, updatedOrg)
}
...
func checkUpdateTraceable(t *testing.T, stub *shim.MockStub, orgAsJSON []byte, value Traceable) {
	res := stub.MockInvoke("1", [][]byte{[]byte("updateTraceable"), orgAsJSON})
	if res.Status != shim.OK {
		fmt.Println("failed", string(res.Message))
		t.FailNow()
	}
